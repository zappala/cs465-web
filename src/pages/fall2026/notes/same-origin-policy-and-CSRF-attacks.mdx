...
- a collection of policies, really

- DOM

  - A script from origin A can't access the DOM of a page from origin B
  ...
  - images, stylesheets may be loaded from other origins

  - scripts
    - origin A may load scripts from other origin B, but they operate in the context of origin A,
      origin A can't read the contents of the script from origin B
    - fetch requests: A script from origin A can't fetch data from origin B unless
  explicitly allowed by the server for origin B


<Image
  alt="Figure 9.5, showing how the same origin policy works"
  src={figure_9_5}
/>
